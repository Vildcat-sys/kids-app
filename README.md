...
共 **142 个知识点**：生活科普 / 地理军事 / 人文科技各 24 个，逻辑思维 / 空间思维各 22 个，**英语 26 个**。题型分布：三选一 94 / 排序 16 / 配对 18 / 听音选词 14。年龄档位：3-5 档 84 个 / 6-8 档 139 个。下表为各领域的代表性内容。
...
绘本卡片回答「这是什么」，探究课堂回答「**我怎么自己把它弄明白**」。
这是对标参考产品之后补上的一层：它的课不是「知识点 + 一道题」，
而是一节走完五个阶段的探究课。骨架还原见 `docs/ARCHITECTURE`。

一节课 **5 阶段 10 步**，入口在绘本卡片下方的「探究课堂」卡片 ——
只有 `src/data/lessons.js` 里写了的条目才有（其余条目照旧走绘本）。

| 阶段 | 步骤 | 做什么 |
|---|---|---|
| 引入 | 主动探索 | 小狮子奇奇带出问题，列 3 个生活观察点 |
| 引入 | 提问猜想 | 先猜一个答案 —— **不判对错**，等实验自己揭晓 |
| 探究 | 收集准备 | 材料清单（手电筒 / 小玩具 / 白墙），点齐才能继续 |
| 探究 | 动手实验 | 分 3 步引导，每步做完揭晓一个现象，可**拍照记录** |
| 探究 | 互动问答 | 实验之后提问 —— 这一步才第一次判对错 |
| 应用 | 生活中的应用 | 日晷 / 皮影戏 / 手影游戏 |
| 应用 | 知识拓展 | 「为什么影子一天里会跑」 |
| 表达 | 当小老师 | 用自己的话讲一遍，可**录音** |
| 检验 | 单元测试 | 2~4 题，逐题判分 |
| 检验 | 探索报告 | 星星 + 金币 + 掌握度，接 `ui/reward.js` 的彩带庆祝 |

**几条内容纪律**（写在 `src/data/lessons.js` 文件头）：

1. **实验必须家里真能做**，材料不超过三样。做不到的宁可不写 —— 让孩子做不成，比不做更伤。
2. **猜想不判对错**。低龄孩子的「先猜再验」是科学思维的起点，不能变成又一道题。
   `tests/lesson.test.mjs` 里有一条断言专门守着：`conjecture` 一旦出现正确答案字段就报错。
3. **表达环节给句式脚手架，不给标准答案**。目标是「敢说」，不是「背对」。
4. **评星永远不给 0 星**。一题没对也给 1 星 —— `starsFor()` 单独测了这条。

**两个真设备能力的处理**：

- **拍照**：`<input type="file" accept="image/*" capture>` —— 手机上直接开相机，
  桌面浏览器退化成选文件。**不需要原生插件，也不用申请权限**。
- **录音**：优先 `MediaRecorder`；拿不到麦克风或被拒绝时**不报错、不拦路**，
  改成「先对爸爸妈妈讲一讲」，孩子照样能往下走。

**加一节课**：在 `src/data/lessons.js` 里按 key（= 知识点 id）写内容即可，界面一行不动。
`lessonSteps()` 按写了哪些环节自动推导步骤 —— 只写引入也能跑（那就 2 步：引入 + 报告）。
...
把 `store` 里已经记下的数据（各域掌握率、该温习数、最近学习时间线）聚合成一个**本地只读**视图，零外部上报 —— 和「隐私只存本地」的立场一致。

- **入口**：顶栏「家长」按钮 → `#/parent`
- **总览**：已掌握 X/Y、该温习 Z 个、总正确率（尝试次数 / 答对次数）
- **分领域表**：每个领域的掌握率（已点亮/总量）、正确率（答对/作答）、进度条
- **最近学习**：按 `learnedAt` 倒序的时间线，每条显示「知识点名 · 领域 · 多久以前」
- **该温习清单**：当前超过 7 天间隔的已点亮项，浅橙底突出
- **隐私横幅**：页面顶部明确写「本页数据只存在这台设备里，不上传任何地方」
- **清空进度**：confirm 二次确认后 `store.reset()`，可重头开始（给二胎 / 借别人玩用）
- **实现纪律**：聚合逻辑是纯函数 `buildParentReport(store, topics, now)`（可单测），渲染走既有 `h()`；计算全在客户端，不发任何网络请求

> 顶栏「家长」入口目前**没有儿童锁**（v1 先能看，锁是下一轮）。
...
> **别只看「占比」这个指标。** 6-8 档占 97% 看着像「高龄端没分级」，但那是因为 3-5 档的内容本来就该是高龄档的**子集** —— 低龄能看的高龄也能看，反过来不成立。真正该看的是**独占数**：高龄档里有 36 个（41%）是低龄档看不到的，这才是切档的可见差异。
...
| 项目 | 结果 |
|---|---|
| 内容校验 | 6 领域 / **88 知识点** / 88 题 / 88 插画，全部通过（零警告） |
| 单元测试 | 99 条，全部通过（含家长端 `buildParentReport` / `timeAgo` 5 条纯函数测试） |
| 题型分布 | choice 52 / order 15 / match 13 / listen 8 |
| 年龄档位分布 | 3-5 档 **52** 个（独占 3）/ 6-8 档 **85** 个（独占 36）/ 全部 88 个 |
| **英语补高龄内容** | 英语 3-5 档 12 个 / 6-8 档 **17** 个，含 5 个 6-8 档专属（自然拼读 / 短句 / 方位词 / 疑问词 / 时间词） |
| 首页渲染 | 1024 宽下 2 列布局，6 个领域卡片，tagline 无折行 |
| 领域页渲染 | 生活科普 / 逻辑思维各 14 个知识点卡片，本次新增 23 张插画全部可辨认 |
...
| **领域页按档过滤** | 逻辑思维 3-5 档 8 个（含「数数」）/ 6-8 档 12 个（含「守恒 / 包含 / 传递」）；英语 3-5 档 12 个 / 6-8 档 17 个，档位胶囊正确显示 |
| 素材采集 | 6/6 源成功（GBIF / Open-Meteo / USGS / Launch Library / NASA / Datamuse） |
| **APK 出包** | 首次 `BUILD SUCCESSFUL in 7m 59s`；加年龄分级后重出 **`in 12s`**；加「我的地图」 **`in 13s`**；加复习机制 **`in 12s`**；复习主动化 + 地形装饰 **`in 12s`**；内容扩充（+23 条）重出 **`in 27s`**；**家长端（成长报告）重出 `in 11s`（3 executed / 90 up-to-date）** —— 三步链路 `npm run www` → `npx cap copy android` → `gradle assembleDebug` 跑顺了 |
| APK 包信息 | 包名 `com.xiaoxiaobaike.app`、compileSdk **36**、minSdk **24**、targetSdk **36**、应用名「小小百科」，体积 **4.07 MB** |
| APK 签名 | v2 方案验证通过，debug 证书 `CN=Android Debug`，RSA 2048 |
| APK 内容 | **88 知识点 / 35 个 web 资源**（新增 `ui/parent.js`）。开包逐项验证 **69 项全过**：含「我的地图」路由 + 主题分组 + 撞色修复 + store v3 复习机制 + 地图 4 态 + 复习提示 + 首页复习主动化（橙色卡片 + 副文案）+ 岛屿地形装饰 + 本次扩充的 23 条新知识点与新插画 + **家长端**（路由 `#/parent` + 顶栏入口 + `buildParentReport`/`timeAgo` 纯函数 + 隐私横幅 + 重置按钮 + store `attemptsOf`/`correctOf` + sw 缓存 v8） |
| **未验证** | **真机安装** —— `adb devices` 无设备连接，装机与实机运行待补 |

截图存于 `.preview/`：`v3-science.png`、`v3-logic.png`、`v3-card-daynight.png`、`v3-card-panda.png`、`v3-test-quiz.png`、`age-ask.png`、`age-3-5.png`、`age-6-8.png`、`age-topic-3-5.png`、`age-topic-6-8.png`、`v5-english-6-8-full.png`、`v5-english-3-5.png`、`v5-art-all.png`、`v6-map-all-20.png`、`v6-map-phone.png`、`v6-home-entry.png`、`v7-map-review.png`、`v7-map-mixed.png`、`v8-map-terrain.png`、`v8-home-review.png`、`v8-home-no-review.png`、`v9-parent.png`。
...
2. **内容量是入门规模** —— 共 88 个（英语 18 个，其余五领域各 14 个），够孩子玩一阵，
   但离「百科」还差得远。扩充成本很低（加知识点只改 `src/data/*.js`），
   真正的瓶颈在**内容审校**，不在工程
3. **年龄分级的低龄端还是偏多** —— 3-5 档 52 个，对 3 岁孩子仍然多。
